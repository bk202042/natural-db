...
CREATE TABLE IF NOT EXISTS memories.fees (
    ...
    fee_type TEXT NOT NULL CHECK (fee_type IN ('electricity', 'management', 'water', 'rent', 'gas', 'internet', 'maintenance', 'insurance', 'parking', 'other')),
    amount NUMERIC(10, 2),
...
CREATE TABLE IF NOT EXISTS memories.documents (
    ...
    tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    chat_id TEXT NOT NULL,
    doc_type TEXT NOT NULL CHECK (doc_type IN ('lease_agreement', 'rental_contract', 'utility_invoice', 'maintenance_invoice', 'insurance_document', 'inspection_report', 'other')),
